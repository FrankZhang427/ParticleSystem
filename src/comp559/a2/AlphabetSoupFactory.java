...
public class AlphabetSoupFactory {
    ...
    private IntParameter which = new IntParameter( "which font", 0, 0, 73);
    private DoubleParameter size = new DoubleParameter( "size", 200, 10, 1000 );
    private BooleanParameter bold = new BooleanParameter( "bold", false );
    private BooleanParameter italic = new BooleanParameter( "italic", false );    
    private DoubleParameter offsetx = new DoubleParameter("x offset", 50, 0, 400 );
    private DoubleParameter offsety = new DoubleParameter("y offset", 250, 0, 400 );    
    private IntParameter segmentBreak = new IntParameter("approximate segment length", 10, 5, 100 );    
    ...
    public void createLetter( ParticleSystem system, String letters, double offx, double offy ) {
        ...
        // perhaps not too efficient... actually very inefficient
    ...
    private Particle p2, p1, p0;

    /** 
     * A helper list to keep track of the particles we've created for this particular
     * string so far... we'll not want to create particles for the same position twice!
     */
    private List<Particle> newParticles = new LinkedList<Particle>();
    ...
    private void addParticle( double x, double y, double offx, double offy, ParticleSystem system ) {        
        ...
        if ( p0 != null ) system.leafSprings.add( new LeafSpring( p0, p1, p2 ) );
    }
    
    /**
     * Checks to see if particles are within a 4 pixel threshold distance.
     * If they are, then that particle is returned, otherwise null.
     * @param x
     * @param y
     * @return either null or a particle within 4 pixels
     */
